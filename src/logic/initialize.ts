export default async function initialize()
{
    ...
    await invoke<Commands.Response["get_computer_info"]>('get_computer_info').then((response) => {
        console.log('computer_info :', response)

        store_Device.computer_info = response
    })

    await invoke<Commands.Response["get_device_info"]>('get_device_info').then((response) => {
